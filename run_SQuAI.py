...
Answer:"""
    ...
    def _create_agent3_prompt(self, query, document, answer):
        """Agent-3 prompt: chunk-level relevance scoring"""
        return f"""You are a strict relevance evaluator.
Given a retrieved evidence chunk (Document), a Question, and an LLM Answer, estimate how relevant and supportive the document is for the question and answer.

Output MUST be valid JSON on a single line with exactly these keys:
{{"relevance_score": <float between 0 and 1>, "reason": "<short reason>"}}

Scoring guidance:
- 0.0-0.2: unrelated / misleading
- 0.3-0.5: weakly related, insufficient evidence
- 0.6-0.8: relevant and partially supportive
- 0.9-1.0: highly relevant and directly supportive
...
Return JSON only."""
    ...
    def _parse_agent3_score(self, text: str) -> Tuple[float, str]:
        """Parse Agent3 JSON output: {"relevance_score": 0-1, "reason": "..."}."""
        raw = (text or "").strip()
        reason = ""
        score = None
        try:
            ...
            # Fallback: extract score from free-form output.
            m = re.search(r"relevance[_ ]?score[^0-9]*([01](?:\.\d+)?)", raw, re.IGNORECASE)
            if m:
                try:
    ...
    def _process_single_question(self, query: str, db=None) -> Tuple[List[Tuple], List[Tuple[str, str, float]]]:
        ...
        with time_block(f"agent3_evaluation_{query[:20]}"):
            ...
            for _, _, _, judge_document, answer in tqdm(doc_answers):
                ...
                score_text = self.agent3.generate(prompt, max_new_tokens=128)
                score, reason = self._parse_agent3_score(score_text)
                scores.append(score)
        ...
        sigma = np.std(scores)
        logger.info(
            f"Agent3 relevance stats for '{query[:30]}...': mean={tau_q:.4f}, std={sigma:.4f}"
        )

        # Step 5: Rank chunks by Agent3 relevance score
