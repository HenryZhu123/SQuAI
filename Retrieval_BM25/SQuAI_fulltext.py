#!/usr/bin/env python3
"""4-Agent RAG System with BM25-Only Retrieval and Question Splitting"""
import plyvel
import argparse
import json
import time
...
class FourAgentRAG:
    """4-Agent RAG System with BM25-Only Retrieval and Full-Text Context Strategy"""
    ...
    def __init__(self, retriever, agent_model=None, n=0.0, falcon_api_key=None, index_dir="test_index", max_workers=4, max_context_chars=100000):
        """Initialize with 4-agent architecture and BM25-only retrieval"""
...
Write your answer now with proper individual citations after each claim:"""
    ...
    def answer_query(self, query, db=None, choices=None):
        """Process query with BM25-only retrieval and full-text context strategy"""
...
def format_result_to_schema(result):
    ...
    formatted_result = {
        ...
        "retriever_type": result.get("retriever_type", "bm25"),
...
def main():
    """Main function with BM25-only 4-agent support"""
    parser = argparse.ArgumentParser(description="4-Agent RAG with BM25-Only Retrieval and Question Splitting")
    parser.add_argument("--model", type=str, default="deepseek-chat", help="LLM id (DeepSeek API or local HF); use DEEPSEEK_API_KEY for API")
    parser.add_argument("--n", type=float, default=0.5, help="Adjustment factor for adaptive judge bar")
    parser.add_argument("--retriever_type", choices=["bm25"], default="bm25", 
                        help="Type of retriever (BM25-only)")
    ...
    parser.add_argument("--max_context_chars", type=int, default=100000, help="Maximum context characters")
    ...
    # BM25-only retriever (no conflicts)
