...
Answer:"""
    ...
    def _clean_document_text(self, text: str) -> str:
        """Clean document text and EXTRACT abstract content instead of removing it"""
        import re

        # Step 1: Extract the actual abstract text from the JSON-like structure
        ...
        if match:
            ...
            # Get the title (everything before the JSON structure)
        ...
        # Step 2: Clean up the extracted content (keep the rest of your cleaning)
        text = re.sub(r"\\[a-zA-Z]+\{[^}]*\}", "", text)
        text = re.sub(r"\$[^$]*\$", "[MATH]", text)
        text = re.sub(r"\s+", " ", text)
        text = re.sub(r"\n\s*\n", "\n", text)

        # Optional: Remove the 500 character limit
        # if len(text) > 500:
        #     text = text[:500] + "..."
...
def main():
    parser = argparse.ArgumentParser(description="BM25-Only Basic RAG")
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        choices=["bm25"],
        default="bm25",
        help="Retrieval strategy: bm25 (BM25 only)",
    )
    parser.add_argument(
        "--alpha",
        type=float,
    ...
    if USING_CONFIG:
        ...
        logger.info(f"Using config.py BM25 index: {bm25_index_dir}")
    else:
        bm25_index_dir = args.bm25_index_dir
        logger.info(f"Using command line BM25 index: {bm25_index_dir}")

    logger.info(f"Initializing BM25-ONLY retriever...")
    retriever = create_bm25_only_retriever(
        bm25_index_directory=bm25_index_dir, top_k=args.top_k
    )
    ...
    logger.info(f"Initializing BM25-only RAG with top_k={args.top_k}...")
    llm_key = (
        args.falcon_api_key
    ...
    if args.single_question:
        logger.info(
            f"Processing single question with BM25-ONLY: {args.single_question}"
        )
